...
def clustering(matrix, n_clusters=20):
...
def run(culture, v, data_type='wvq', cur_model='gpt-3.5-turbo-0613', g_num=500):
    ...
    with jsonlines.open(f'{dir_path}/WVQ_cross_coversation_{g_num}_{v}.jsonl',mode='a') as writer_c:
        with jsonlines.open(f'{dir_path}/WVQ_cross_{g_num}.jsonl',mode='a') as writer:
            with open(path_dict[data_type], "r+", encoding="utf8") as f:
                for item in jsonlines.Reader(f):
                    ...
                    if '?' not in content and data_type == 'wvq':
                        background = f"{boy} is a {people} boy and {girl} is an American girl. {boy} {attitude} with the idea that {original_opinion}. All of {boy}'s statements should conform to the thinking. All of {girl}'s statement should conform to his culture."
                        history = [{"role": "system", "content": background}]
                    elif (data_type == 'wvq' and len(option) == 0) or data_type == 'specific':
                        background = f"{boy} is a {people} boy and {girl} is an American girl. When asked {original_opinion}, {boy} answered that {attitude}. All of {boy}'s statements should conform to the thinking. All of {girl}'s statement should conform to his culture."
                        history = [{"role": "system", "content": background}]
                    else:
                        background = f"{boy} is a {people} boy and {girl} is an American girl. When asked {original_opinion}, Option: {option}, {boy} answered that {attitude}. All of {boy}'s statements should conform to the thinking. All of {girl}'s statement should conform to his culture."
                        history = [{"role": "system", "content": background}]
                    ...
                    if v == 'v1':
                        ...
                        sys = "Do you agree with her? Provide more reasons to support your idea."
                        response_1, history = getResponse(sys, cur_model, boy, history)
                        print(sys)
                        print(f'{boy}: ', response_1)
                        conversation_dict['content'].append('System: ' + sys)
                        conversation_dict['content'].append(f'{boy}: ' + response_1)
                        sys = "What's your opinion? Please provide your reasons."
                        response_2, history = getResponse(sys, cur_model, girl, history)
                        print(sys)
                        print(f'{girl}: ', response_2)
                        conversation_dict['content'].append('System: ' + sys)
                        ...
                        sys = "Talk more about opinions on your culture."
                        response_1, history = getResponse(sys, cur_model, boy, history)
                        print(sys)
                        print(f'{boy}: ', response_1)
                        conversation_dict['content'].append('System: ' + sys)
                        ...
                        sys = f"Are there anything in your culture you want to introduce to {girl}"
                        response_1, history = getResponse(sys, cur_model, boy, history)
                        print(sys)
                        print(f'{boy}: ', response_1)
                        conversation_dict['content'].append('System: ' + sys)
                        ...
                        sys = f"Are there anything in your culture you want to introduce to {boy}"
                        response_1, history = getResponse(sys, cur_model, boy, history)
                        print(sys)
                        print(f'{boy}: ', response_1)
                        conversation_dict['content'].append('System: ' + sys)
                    ...
                    for statement in history:
                        if statement['role'] == 'system' and f"{boy} said" in statement['content']:
                    ...
                    if g_num == 500:
                        ...
                        labels = clustering(embeddings, min(15, len(embeddings)))
                    if g_num == 1000:
