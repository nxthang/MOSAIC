...
iw_df = pd.read_csv(os.path.join("cultural_human_adapt/data/", "country_iwlist.csv"))
iw_bins = list(iw_df['iw_bin'].unique())
...
for j, suffix in enumerate(file_suffixes): # degree
    ...
    for i, prefix in enumerate(file_prefixes.keys()): # model
        ...
        data = pd.merge(data, iw_df, left_on="Country", right_on="country", how="inner")

        for k, bin in enumerate(iw_bins): # subaxes
            subset = data.loc[data['iw_bin'] == bin]
            ...
            f1 = f1_score(subset['Gold Label'], subset[col], average='weighted')
            precision = precision_score(subset['Gold Label'], subset[col], average='weighted')
            recall = recall_score(subset['Gold Label'], subset[col], average='weighted')

            results_df.loc[count, "model"] = prefix 
            results_df.loc[count, "bin"] = bin
    ...
    results_df.to_csv("output_bins/{}_results_weighted.csv".format(suffix.replace("etiquette_", "")))
    grouped_df = results_df.groupby(['bin'])['f1'].mean()
    grouped_df.to_csv("output_bins/grouped_{}_results_weighted.csv".format(suffix.replace("etiquette_", "")))
print(results_df)
