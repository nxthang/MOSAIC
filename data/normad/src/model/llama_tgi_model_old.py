    ...
    def forward(self, X: List, max_tokens: int, batch_size: int, temperature: int, cot: bool = False) -> List:
        ...
        if self.cot:
            ...
            if not self.chat:
                ...
                prompts = self.create_prompts(X, 'llama-2', prefix='', to_extract_likelihoods=True)
            else:
                prompts = self.append_instruct_prompt(model_name='llama-2', text_prompts=X, next_responses = cond_responses_expl, assistant_suffix='# Answer', custom_template=custom_template)
            pdb.set_trace()
            self.cot = False
