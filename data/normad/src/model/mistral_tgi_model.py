...
class LlamaTGIInferencer(BaseModel):
    ...
    def forward(self, X: List, max_tokens: int, batch_size: int, temperature: int, cot: bool = False, to_extract_likelihoods: bool=False) -> List:
        ...
        if self.chat:
            prompts = self.create_instruct_prompts(X, 'llama-2', custom_template=custom_template, assistant_prefix="# Answer", to_extract_likelihoods=to_extract_likelihoods)
        else:
            prompts = self.create_prompts(X, 'llama-2', prefix ='# Answer', to_extract_likelihoods=to_extract_likelihoods)
