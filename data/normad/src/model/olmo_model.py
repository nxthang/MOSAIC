...
class  OLMoTGIInferencer(BaseModel):
    ...
    def __call__(self, X: List, max_tokens: int, batch_size:int,  temperature: int=0, cot: bool=False):
        return self.forward(X, max_tokens, batch_size, temperature, cot=cot)
    ...
    def __get_probabilities(self, responses):
        ...
        for p_tokens in prefill:
            ...
            if text in ['Yes', 'No']:
                ...
                tokenized_neither = self.tokenizer.tokenize("Answer\nNeither", add_special_tokens=False) # apparently, the newline tokenizes differently
    ...
    def get_predictions(self, prompts, max_tokens, batch_size, temperature=0) -> Tuple[List[str], List[List[float]]]:
    ...
    def forward(self, X: List, max_tokens: int, batch_size: int, temperature: int, cot: bool = False) -> List:
        '''
        ...
        if self.cot:
            if self.chat:
                prompts = self.create_instruct_prompts(X, 'olmo', custom_template=custom_template, assistant_prefix="# Explanation", to_extract_likelihoods=False)
            else:
                prompts = self.create_prompts(X, 'olmo',prefix='', to_extract_likelihoods=False)
            ...
            if not self.chat:
                ...
                prompts = self.create_prompts(X, 'olmo', prefix='', to_extract_likelihoods=True)
            else:
                prompts = self.append_instruct_prompt(model_name='olmo', text_prompts=X, next_responses = cond_responses_expl, assistant_suffix='# Answer', custom_template=custom_template)

            self.cot = False
        
        if self.chat:
            prompts = self.create_instruct_prompts(X, 'llama-2', custom_template=custom_template, assistant_prefix="# Answer", to_extract_likelihoods=True)
        else:
            prompts = self.create_prompts(X, 'llama-2', prefix ='# Answer', to_extract_likelihoods=True)
        # get llama predictions
        cond_responses, cond_probs = self.get_predictions(prompts, batch_size=batch_size, temperature=temperature, max_tokens=max_tokens)

        if self.cot:
            cond_responses = [c_e + '# Answer\n' + c_r for c_r, c_e in zip(cond_responses, cond_responses_expl)] 
