...
# Story
{{story}}
Provide an '# Answer' with only either Yes, Neutral, OR No'
"""
...
def get_country_conditioned_prompt(X: pd.DataFrame) -> List[str]:
    condition = "country"
    return [prompt_template.replace("{{condition}}", condition).replace("{{context}}", country.strip()).replace("{{story}}", story.strip()) for story, country in zip(X['Story'], X['Other Country'])] ## Other country = country for non-neutral labels, and is different for neutral
